...
const SkillChip = ({ label, Icon }: SkillChipProps) => {
  return (
    <span className="inline-flex font-sans items-center gap-2 rounded-full bg-[#DFF0FF] px-3 py-1.5 text-sm text-[#183759] shadow-[0_1px_0_rgba(15,23,42,0.04)]">
  ...
  return (
    <Layout>
      <Container>
        ...
        <P className="mt-5 max-w-3xl font-sans text-color1">
      ...
      <Section>
        <Container className="mt-10 space-y-10">
          {skillGroups.map((group) => (
            <Container key={group.title} className="border-t border-slate-200 pt-8">
              <Container className="grid gap-6 lg:grid-cols-[220px_1fr] lg:gap-10">
              <H2 className="text-base font-medium uppercase tracking-[0.18em] text-color2 font-sans">
                  {group.title}
                </H2>
      ...
      <Section>
        <Container className="mt-12 space-y-8">
          <Container className="border-t border-slate-200 pt-8">
            <Container className="grid gap-6 lg:grid-cols-[220px_1fr] lg:gap-10">
              <H2 className="font-sans text-base font-medium uppercase tracking-[0.18em] text-color2">
                AI IN MY WORKFLOW
              </H2>
              ...
              <Container className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                {workflowCards.map(({ title, Icon, bullets }) => (
                  ...
                  >
                    <div className="flex items-center gap-2 font-sans text-base font-medium">
                      <Icon className="h-5 w-5 shrink-0" strokeWidth={2} />
                      <span>{title}</span>
                    </div>

                    <ul className="mt-4 space-y-2 pl-1 font-sans text-sm leading-6">
                      {bullets.map((bullet) => (
